...
export function SertifikatEditForm({ detail }: { detail: Certificate }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = mapToSertifikasiPayload(rawData, mahasiswa.items, dosen.items);

    try {
      const response = await sertifikasiService.updateSertifikasi(
        detail.id,
        payload,
      );
      if (response.success) {
  ...
  return (
    ...
    >
      ...
      <CertificateDetailSection defaultData={detail} />
      ...
      <FormFooter
        backHref={`/certificate/${detail.id}`}
