import { Award, Package, Home, Link as LinkIcon, NotebookPen } from "lucide-react";
    ...
    <Card className='w-full shadow-sm rounded-2xl border-slate-200 overflow-hidden bg-white'>
      <CardContent className='p-6 md:p-8 flex flex-col gap-5'>
        <div className='flex items-center gap-3'>
          ...
          <div className='flex flex-col gap-0.5'>
            ...
            <p className='text-xs text-slate-500'>
              Informasi rekognisi, publikasi, atau karya yang diakui.
            </p>
          </div>
        </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          ...
          <div className='flex flex-col gap-1.5'>
            <Label htmlFor='nama-rekognisi' className={LABEL_CLASS}>
              Nama Rekognisi <Required />
            </Label>
            <IconInput
              id='nama-rekognisi'
              icon={Package}
          ...
          <div className='flex flex-col gap-1.5'>
            <Label htmlFor='jenis' className={LABEL_CLASS}>
              Jenis <Required />
            </Label>
            <Select required>
              <SelectTrigger id='jenis' className={SELECT_CLASS}>
                <SelectValue placeholder='Pilih Jenis' />
              </SelectTrigger>
              <SelectContent className='rounded-xl border-slate-200'>
                {JENIS_REKOGNISI_OPTIONS.map((opt) => (
                  <SelectItem key={opt} value={opt}>
                    {opt}
                  </SelectItem>
                ))}
          ...
          <div className='flex flex-col gap-1.5 md:col-span-2'>
            <Label htmlFor='url-rekognisi' className={LABEL_CLASS}>
              URL Rekognisi <Required />
            </Label>
            <IconInput
              id='url-rekognisi'
              icon={LinkIcon}
          ...
          <div className='flex flex-col gap-1.5'>
            <Label htmlFor='link-bukti' className={LABEL_CLASS}>
              Link Dokumen/Undangan/Bukti Lain
            </Label>
            <IconInput
              id='link-bukti'
              icon={LinkIcon}
